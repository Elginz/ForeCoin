...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateDashboardUI(data) {
        ...
        // Update model prediction values and signals (different from stable coins)
    ...
    function updateSignal(signalElementId, lastClose, predictedClose) {
        ...
        // Set the text and color based on the prediction, now including the percentage
        if (predictedClose > lastClose) {
            element.textContent = `UP (${change.toFixed(2)}%)`;
            element.className = 'badge bg-success';
        } else if (predictedClose < lastClose) {
            element.textContent = `DOWN (${change.toFixed(2)}%)`;
            element.className = 'badge bg-danger';
        } else {
            element.textContent = `HOLD (${change.toFixed(2)}%)`;
    ...
    if (searchForm && searchInput && contentCards.length > 0) {
        searchForm.addEventListener('submit', function(e) {
            ...
            if (foundCard && matchInfo) {
                // If content is found, smoothly scroll to it
